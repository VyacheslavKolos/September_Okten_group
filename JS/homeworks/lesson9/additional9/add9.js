...
for (const user of usersList) {
    ...
    for (let userKey in user.address) {
        if (userKey !== "geo") {
            ...
            street.innerText = `${userKey}- ${user.address[userKey]}`;
            div.appendChild(street)
        } else {
            let location = document.createElement("h4");
            location.innerText = `lat ${user.address.geo.lat}`;
            div.appendChild(location)

            let lng = document.createElement("h4");
    ...
    for (let aboutCompany in user.company) {
        let companY = document.createElement("h4");
        companY.innerText = `${aboutCompany} - ${user.company[aboutCompany]}`;
